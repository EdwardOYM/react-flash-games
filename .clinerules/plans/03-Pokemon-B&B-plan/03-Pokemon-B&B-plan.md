...
- [x] 10 — End flow + polish + docs (victory/defeat, highscores, settings, responsive, credits, diagrams, final validation)
  ...
  - [x] **CP10-B** — new `src/games/pokemon-bnb/highscores.ts` mirroring `tron/highscores.ts`: `readHighscores()` (bucket `highscores['pokemon-bnb']`, top-10 by wins) and `recordMatchWin(name)` aggregating per player name case-insensitively; one record per match, winner only. No schema change (bucket already seeded); `database-schema.md` row lands in CP10-F. (Done: verbatim Tron mirror over bucket `'pokemon-bnb'` — `readHighscores` reads via `readConfig()` + top-10 slice, `recordMatchWin` aggregates case-insensitively through `updateConfig` and sorts desc/caps 10; empty/whitespace name is a no-op returning the read. All persistence funnels through `src/config/index.ts`; bucket seed verified at `default.config.json` line 28 (`"pokemon-bnb": []`). No consumer yet — `recordMatchWin` is called by CP10-C. Validation: `npm run build` + `npm run lint` clean (0 warnings / 0 errors).)
  - [x] **CP10-C** — end flow in `PokemonBnbGame.tsx`: `battle.over` drives `victory` (local seat won) / `gameover` (lost) once, `recordMatchWin` fires once per match for the winner, `highscore` view with the shared `HighscoreTable` (translated labels), and every results view leaves: rematch (guest offer / host accept, reusing the CP9-E handshake), retry (fresh lobby seed → `opening`) and back-to-start (`leaveLobby`). No dead ends; `?local=1` results path included. (Done: a `settleMatchOver(next)` one-shot (via `settleMatchOverRef` + `resultRecordedRef`, both reset in `resetMatchState`) is invoked from the three places `battle.over` can first become true on this seat — the CP8-B driver's success path, the CP9-C timeout macrotask, and the guest's `battle-snapshot` handler — so `recordMatchWin(winnerName)` records exactly once per match per device and `showNotice('highscoreSaved')` confirms it; routing: `?local=1`/winner == local seat → `victory`, else `gameover`, only while `playing`/`paused`. New render branches before the placeholder: results view (title via `victory`/`gameOver`, `resultVictory`/`resultDefeat`, `resultReason`/`resultPrizes`, `bnb-summary` settings replay, CP9-E rematch controls repeated, host `retry` (`startPackOpening` → fresh seed → `opening`) / local `playAgain`, `highscore` + `backToStart` exits) and a `highscore` view (shared `HighscoreTable` with CP10-A labels, `back` returns to the matching results view or `start`); the old six-view placeholder is narrowed to `loading` only, making the previously unreachable `victory`/`gameover`/`highscore` reachable *and* leaveable (the CP10-0 dead-end fixed). Reused existing copy/CSS: `back`, `bnb-copy`/`bnb-summary`/`bnb-rematch`/`bnb-shell`/`bnb-actions` all existed — zero new keys, zero new CSS. Two gate defects found and fixed: `SideState` has no `prizesTaken` (used `prizeCards - prizeCount`, the engine's own formula) and an `exhaustive-deps` warning (added `settleMatchOver` to the timer effect deps). Validation: `npm run build` pass (only the pre-existing >500 kB chunk advisory) + `npm run lint` 0 warnings / 0 errors (43 files).)
  - [x] **CP10-D** — settings + keybindings in `PokemonBnbGame.tsx`: pass `additionalBindings` (`pokemon-confirm` / `pokemon-skip`) into `SettingsModal`, read overrides through `AppConfig.settings.keybindings`, and wire Confirm/Skip to advance/skip the opening ceremony and to confirm the promotion/attack pick; locale changes delegate to `onLocaleChange`. No schema change. (Done: `additionalBindings` = `[{ id: 'pokemon-confirm', labelKey: 'keyNames.confirm', defaultKey: 'Enter' }, { id: 'pokemon-skip', labelKey: 'keyNames.skip', defaultKey: 'S' }]` (ids kept technical, labels translated) now passed to the existing `SettingsModal` render, which persists through `AppConfig.settings.keybindings` — verified against `ControllerSettings`' `additionalBindings` contract, no schema change. A `readBinding(id, fallback)` helper resolves overrides from `settings.keybindings`, and a `keyHandlerRef` (ref pattern: handler re-assigned every render, `keydown` listener bound only while no overlay owns the keyboard — `localMode`/`settingsOpen`/`helpOpen` gate it, `key.repeat` and focused inputs/buttons/textarea/select ignored) drives the shared beats: opening = Skip reveals all / Confirm reveals next, then marks opening-ready; deck = Confirm submits the deck when legality allows; battle = Confirm confirms the promotion gate when a bench pick is staged (attacks stay button-only per CP8-D). Locale changes already delegate to the persisting `onLocaleChange` prop (verified in CP10-0; no code needed). `?local=1` keeps its own harness controls, so keys stay idle there. Initial wiring produced 5 lint warnings (function deps in a per-render rebound effect); fixed by the repo's ref-settle pattern — handler in `keyHandlerRef`, listener effect deps only `[localMode, settingsOpen, helpOpen]`. Validation: `npm run build` pass (pre-existing chunk advisory only) + `npm run lint` **0 warnings / 0 errors** (43 files). No CSS/engine/schema changes.)
  - [x] **CP10-E** — responsive + polish in `PokemonBnbGame.tsx` / `PokemonBnbGame.css`: results/highscore/settings styling, touch-friendly action bar, 960×540 embed cap + `≤520px` portrait + mobile-landscape audit (no horizontal scroll), and credits/`inspiredBy` verification (entry already auto-listed). (Done: results/highscore shells scoped with `bnb-results` / `bnb-highscore` modifiers (tsx), then CSS-only polish — `.bnb-rematch button` + focus/`.bnb-primary` styles added because the rematch buttons in the results view had no button rule (the CP10-C gap); results summary/actions rhythm tightened; landscape block gains compact copy/summary margins and `.bnb-highscore .highscore-table { max-height: max(120px, calc(100dvh - 244px)); overflow-y: auto }` so the ≤10-row table can never be clipped by the hidden-scroll landscape page (verified `HighscoreTable.css` ships no internal scroll); portrait block gains 42px touch targets (`bnb-actions`/`bnb-rematch`/`bnb-topbar` buttons), a 2-column results summary, and 44/1fr/56px highscore columns so names ellipsize instead of forcing h-scroll. 960×540 cap untouched (`min(100%, 960px)` / `16/9` battle stage); settings styling already covered by the existing `bnb-fields`/`bnb-overlay-card` + landscape stack rules. Credits verified in CP10-0: `CreditsPage` auto-lists the registry entry with `gameInspiration.pokemonBnbMini` present in en/ms/zh — no change needed. Audit by code at 960×540 / ≤520px portrait / mobile landscape: no horizontal scroll paths (all new grids `minmax(0, …)`). Validation: `npm run build` pass (pre-existing chunk advisory only), `npm run lint` 0 warnings / 0 errors, `git status` = plan + tsx + css.)
  - [x] **CP10-F** — final validation (`npm run build` + `npm run lint` + throwaway key-parity/`?local=1` end-flow regression, deleted after) + docs/diagrams: `.github/diagram/architecture-flow.md` (results edges solid, view union note, persistence call-site row) and `.github/diagram/database-schema.md` (`GAME_CATALOG` pokemon row + `highscores['pokemon-bnb']` bucket), then mark CP10 (and the whole plan) complete. (Done — **CP10 and the whole 03-Pokemon-B&B plan are complete**. Gates: `npm run build` clean (only the pre-existing >500 kB chunk advisory); `npm run lint` 0 warnings / 0 errors. Throwaway `scripts/cp10f-parity.mjs` (deleted after the run): flat keys **394/394/394** en/ms/zh, `pokemonBnb` **216/216/216**, pairwise missing **0**, **166 literal `t()` uses / 103 unique all resolving**. Throwaway `scripts/cp10f-regress.mjs` (esbuild-bundled against the real engine + 30C set data, deleted after the run): **22/22 assertions passed** — setup invariants (`prizeCards`/`timerSeconds` honored from lobby settings, fresh battle has no winner), the guest render contract (`applySnapshot(toSnapshot(state,'guest'))` is view-only yet carries `over`/`winner`/`winReason`/`prizeCards`, which is exactly what the CP10-C results guard and settle path read), a real `endTurn` deck-out driving `over` + `winner:'host'` + `winReason:'deck-out'`, `match-over` refusals afterwards, the final snapshot round-trip preserving the result including the results view's derived prizes-taken formula (`prizeCards − guest.prizeCount`), and `applyTimeout` flipping seat+turn while staying a no-op when the timer is off. Three fixture bugs were hit and fixed during authoring (not engine defects): the engine returns fresh state objects so assertions must thread the returned state, the basic-Pokemon filter needs `stage === 'Basic'` on real set data, and one assertion originally read a snapshot field that does not exist instead of the component's derived formula. Both scripts deleted; `scripts/` back to its tracked contents (`cp8-add-locale.mjs`, `fetch-pokemon-set.mjs`). Two-tab Cloud QA stays manual (not runnable here); those paths are covered by the code-level checks plus the CP9-D re-dial logic. Diagrams: `.github/diagram/architecture-flow.md` — victory/gameover/highscore edges made solid out of the battle state (settle-once call sites: driver success, timeout macrotask, guest snapshot), results ⇄ highscore round trip + highscore → start escape, `recordMatchWin` → config persistence edge, the view-union/prose sections rewritten from "CP10 placeholder" to the shipped reachable+leaveable end flow (one-shot highscore record, confirm/skip keybindings, settings bindings) — and `.github/diagram/database-schema.md` — `GAME_CATALOG` pokemon-bnb row, the `highscores['pokemon-bnb']` bucket entry, and the `pokemon-confirm`/`pokemon-skip` keybindings examples added to the documented `AppConfig` shape (all seeded in `default.config.json` and merged by `mergeConfig`; no code change).)

---

# Milestone 11 — Card images via hosted TCGdex links

> Follow-up to CP10 (plan complete), implementing the CP7 note ("Card images (future)… Track as a
> follow-up task"). **User decision: hotlink, do not bundle** — render card artwork from TCGdex's
> hosted image CDN (`assets.tcgdex.net`, the same open API the set data came from; the earlier
> pokemontcg.io source was swapped to TCGdex at the user's direction after its API proved flaky);
> nothing is downloaded or stored in the repo. Protocol: one
> sub-step per turn + gates, then stop. The game already needs internet for PeerJS lobby play, so
> the hosted-image dependency is acceptable; **offline/error fallback = the existing typographic
> facsimile**, so the game never breaks. Fan-made / non-commercial fair-use posture stands, with
> attribution prominent and a one-commit removal path documented.

- [x] 11 — Card images via hosted TCGdex links (runtime hotlink + facsimile fallback, artwork credits, one-commit removal path)

- [x] **CP11-0** — Breakdown + image-source probe. (Done: this edit + probe run. First probed pokemontcg.io per the original direction — its v2 API proved flaky (intermittent 500/502/504 behind a proxy; site now advertises "part of Scrydex"), so the user switched the source to **tcgdex.net**, which the set data already comes from. Probe results (all verified live): TCGdex set path is **`me/30th`** (series folder + set id — the card id `30th-001` alone is NOT the asset path; the detail endpoint returns `image: "https://assets.tcgdex.net/en/me/30th/<localId>"`); asset pattern **`https://assets.tcgdex.net/en/me/30th/<localId>/<quality>.png`** with quality ∈ `low` / `high`; **padded localIds work as-is** ("001".."158" — the same values already stored in `cards.json` `number`, zero conversion needed); set parity **158/158 exact** vs `cards.json`; HEAD-checked 001/023/052/126/158 (incl. a trainer, 126 Poké Pad) all HTTP 200 `image/png`; sizes: `low` ≈ 47–62 KB, `high` ≈ 210–323 KB → **`low` chosen** (plenty for the ~245px card frame, ~5× cheaper on bandwidth; `high` documented as an optional upgrade). Synthetic basic-energy cards have no TCGdex equivalents → facsimile by design. `PokemonCard.tsx` is the single render point for every card face (ceremony, tabletop, deck builder) so one integration covers all views.)
  - [x] **CP11-A** — new `src/games/pokemon-bnb/cardImage.ts`: `const TCGDEX_SET_PATHS: Record<string, string> = { '30c': 'en/me/30th' }` + `cardImageUrl(card: CardDef, quality: 'low' | 'high' = 'low'): string | undefined` — returns `` `https://assets.tcgdex.net/${path}/${card.number}/${quality}.png` `` for set cards (`card.number` is already the zero-padded TCGdex localId, zero conversion), `undefined` for synthetic energies (`supertype === 'energy'`) / set ids without a mapping / non-numeric numbers (regex guard). Pure URL builder, no state, no fetching logic, set-id registry keyed by game set id so future sets drop in as one line. Validation: `npm run build` clean (pre-existing >500 kB chunk advisory only), `npm run lint` 0 warnings / 0 errors.
  - [x] **CP11-B** — `PokemonCard.tsx` image layer. (Done. **Design deviation from the art-window sketch, recorded deliberately:** a fixed-height art window would have collided with the battle bench's `.pkm-card` min-height: 0 squeeze (CP8-E) and risked overflow/clip in the tight 16:9 stage, so the artwork renders **full-bleed behind the facsimile** instead — `CardArt` (internal component, `useState` failure flag, `key={url}` remount on card change) emits an absolutely-positioned `<img loading="lazy" decoding="async" draggable={false} aria-hidden>` + veil `<i>` inside a `z-index: -1` wrapper; `.pkm-card` gains `isolation: isolate` so the art paints above the card background but below all in-flow text, and the light white veil (`rgba(255,255,255,.45 → .93)` top-to-bottom) keeps the dark facsimile text legible over any artwork; `onError` unmounts the art entirely so the clean facsimile returns (offline-safe, no layout shift since the layer is absolute); face-down backs untouched (early return); damage/status chips stay absolutely positioned above; a11y unchanged (`img` alt=""/aria-hidden, `article` aria-label still the card name; facsimile remains the semantic layer). CSS co-located in `PokemonCard.css` only (4 rules added, no re-declarations). Synthetic energies resolve `undefined` → no art layer at all. Validation: `npm run build` clean (pre-existing chunk advisory only), `npm run lint` 0 warnings / 0 errors.)
  - [x] **CP11-C** — credits + attribution. (Done: `artwork` / `artworkCredit` flat keys added to en/ms/zh (3-way parity **51/51/51**, pairwise missing 0) after the `music`/`musicCredit` credits-group lines; `TranslationKey` union extended in `src/assets/languages/index.ts` (caught by the first build — `t('artwork')` was unassignable until the union grew, same shape as CP10-A); `CreditsPage` gains the "Card artwork" row between Music and Game-inspired-by, routed through `t()` like every other credit; `src/assets/pokemon-bnb/README.md` updated — stale "fully offline-playable / typographic facsimiles only" claims corrected, new Credits/sources bullet documents the hotlink + URL pattern + fallback + fair-use posture, and a "Removing the hosted artwork (one commit)" section documents the exact 3-step strip path. Validation: `npm run build` clean (pre-existing chunk advisory only), `npm run lint` 0 warnings / 0 errors.)
  - [x] **CP11-D** — validation + diagrams (Done: throwaway link-parity probe (HEAD all mapped URLs with a polite delay, deleted after — insurance against numbering drift); `npm run build` + `npm run lint` (bundle delta ≈ zero); viewport pass on the opening ceremony, tabletop, and deck builder (art fills the frame, overlays legible, no horizontal scroll at 960×540 / ≤520px / mobile landscape); `.github/diagram/architecture-flow.md` POKEMON subgraph gains a "card images fetched at runtime from the TCGdex CDN" external-dependency note; **plan complete**. Results: parity probe **158/158 mapped URLs HTTP 200** (`scripts/cp11d-parity.mjs`, deleted after the run — `scripts/` back to its 2 permanent files), `npm run build` clean in 2.15s (pre-existing >500 kB chunk advisory only), `npm run lint` **0 warnings / 0 errors** (44 files), bundle delta **≈ zero** (no assets added — one URL-string module). Viewport confidence is structural, verified by code: the art layer is `position: absolute; inset: 0` inside the otherwise-unchanged `pkm-card` frame, so box metrics are byte-identical at 960×540 / ≤520px portrait / mobile landscape and no horizontal-scroll path is introduced; the only new variables are paint and legibility, both reviewed (white veil .45 → .93 over the dark facsimile text, chips/borders/z-order untouched, face-down backs never mount art, synthetic energies resolve `undefined` by design). Diagram: header note plus the POKEMON edge `pokemonBnb --> cardArt` documenting the runtime TCGdex dependency and the facsimile-only fallback.)
  - [x] **CP11-E** — art-only faces + one uniform card size (user feedback: "Should not show the json texts, thats for the game engine. The cards should all be the same size"). (Done. **JSON text off the face:** `cards.json` content is engine data, so `PokemonCard.tsx` no longer renders the typographic facsimile (name / HP / attacks / rules / illustrator). The face is now only: the hosted TCGdex artwork (`CardArt`), the pre-translated rarity chip, and the CP8 battle overlays (damage counters, status pips). Cards with no hosted art (the 8 synthetic basic energies) or a failed load fall back to a **data-free type tint** (`tintClass`) instead of text — never engine strings. **One uniform size:** `.pkm-card` is a fixed box — `width: var(--pkm-card-w, 168px)` + `aspect-ratio: 8 / 11` (the TCGdex `low` art is 245×337, so `object-fit: cover` never crops) with `flex: none; align-self: start; justify-self: center; margin-inline: auto`, so a card can no longer be stretched or shrunk by its container; the two card grids (`.bnb-card-grid`, `.bnb-bench-list`) place `minmax(var(--pkm-card-w, 168px), 1fr)` tracks with `align-items: start; justify-items: center` so a track can never render a card at a different size. The CP8-E `.bnb-bench-list .pkm-card { min-height: 0 }` squash override — the actual cause of bench faces rendering as slivers beside a full-size active face — was removed. Token overrides are per **view root** in `PokemonBnbGame.css`: `.bnb-battle { --pkm-card-w: clamp(56px, 9vh, 76px) }` (compact so active + 5 bench faces fit the fixed 16:9 stage), `.bnb-page { clamp(72px, 15vh, 132px) }` under mobile landscape, and `.bnb-page { clamp(112px, 34vw, 168px) }` under narrow portrait — within any one viewport every card, active and bench included, is the identical box. Tint coverage verified exhaustive against the `CardType` union (all 10 types + `normal` special-energy + `pokemon`/`trainer` fallbacks). Verified no card face renders any `cards.json` field: `card.name` survives only as UI labelling (aria-labels, deck-builder `+`/`−`, hand-list chips, `<option>` text, and the "X → Y" targeting hints), and no HP / attack / rules prose is rendered anywhere. Validation: `npm run build` clean in 2.12s (pre-existing >500 kB chunk advisory only), `npm run lint` **0 warnings / 0 errors** on 44 files.)
